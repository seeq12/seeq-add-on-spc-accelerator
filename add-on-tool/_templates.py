...
def create_template(URL, display_dict, histogram, input_condition, start_select, end_select, workbook_button, workbook_id, worksheet, histogram_dict):
    ...
    for worksheet_name, item_ids in display_dict.items():
        display_df = spy.search(pd.DataFrame(item_ids, columns =['ID']), quiet=True)[['Name', 'ID', 'Type']].reset_index(drop=True)
        display_df['Lane'] = 1
    ...
    for worksheet in new_worksheet_ids:
        workstep_id = workbooks_api.get_worksheet(workbook_id=workbook_id, worksheet_id=worksheet).to_dict()['workstep'].split('/').pop(-1)
        worksheet_data = workbooks_api.get_workstep(workbook_id=workbook_id, worksheet_id=worksheet, workstep_id=workstep_id).to_dict()['data']
        worksheet_data_dict = json.loads(worksheet_data)
        try:
            worksheet_data_dict['state']['stores']['sqTrendStore']
        except:
            worksheet_data_dict['state']['stores']['sqTrendStore'] = {'labelDisplayConfiguration': {'name': 'off','asset': 'off','assetPathLevels': 1,'unitOfMeasure': 'off','custom': 'off','customLabels': []}, 'showCapsuleLaneLabels': False}
        try:
            worksheet_data_dict['state']['stores']['sqTrendStore']['labelDisplayConfiguration']
        except:
            worksheet_data_dict['state']['stores']['sqTrendStore']['labelDisplayConfiguration'] = {'name': 'off','asset': 'off','assetPathLevels': 1,'unitOfMeasure': 'off','custom': 'off','customLabels': []}
        try:
            worksheet_data_dict['state']['stores']['sqTrendStore']['showCapsuleLaneLabels']
        except:
            worksheet_data_dict['state']['stores']['sqTrendStore']['showCapsuleLaneLabels'] = False
        worksheet_data_dict['state']['stores']['sqTrendStore']['labelDisplayConfiguration']['name'] = 'lane'
        worksheet_data_dict['state']['stores']['sqTrendStore']['labelDisplayConfiguration']['unitOfMeasure'] = 'axis'
        worksheet_data_dict['state']['stores']['sqTrendStore']['showCapsuleLaneLabels'] = True
...
def format_histogram_worksheet(histogram_id, signal_name, URL, workbook_id):
    ...
    workstep_id = workbooks_api.get_worksheet(workbook_id=workbook_id, worksheet_id=histogram_worksheet_id).to_dict()['workstep'].split('/').pop(-1)
    worksheet_data = workbooks_api.get_workstep(workbook_id=workbook_id, worksheet_id=histogram_worksheet_id, workstep_id=workstep_id).to_dict()['data']
    worksheet_data_dict = json.loads(worksheet_data)
    try:
        worksheet_data_dict['state']['stores']['sqTrendTableStore']
    except:
        worksheet_data_dict['state']['stores']['sqTrendTableStore'] = {'items': []}
    worksheet_data_dict['state']['stores']['sqTrendTableStore'] = {'items': [{'autoDisabled': False,
      'stack': False,
      'id': histogram_id,
      'name': 'Histogram',
      'selected': False,
    ...
    workbooks_api.create_workstep(workbook_id=workbook_id, worksheet_id=histogram_worksheet_id,body={"data": replaced_worksheet_data})