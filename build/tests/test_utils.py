...
from build import topological_sort, file_matches_criteria


@pytest.mark.unit
def test_topological_sort():
    graph = {"a": ["b", "c"], "d": [], "b": ["c", "d"], "c": ["d"]}
...
def test_file_matches_criteria():
    ...
    assert file_matches_criteria(
        str(root), str(root / ".config" / "Code" / "User" / "settings.json")
    )
    ...
    file = root / ".config" / "Code" / "User" / ".settings.json"
    with patch(
        "os.stat", return_value=mock.Mock(st_file_attributes=stat.FILE_ATTRIBUTE_HIDDEN)
    ):
        assert file_matches_criteria(str(root), str(file), exclude_hidden_files=False)
        assert not file_matches_criteria(
            str(root), str(file), exclude_hidden_files=True
        )
    ...
    assert not file_matches_criteria(
        str(root), str(file), excluded_folders={str(Path(".config") / "Code")}
    )
    ...
    assert file_matches_criteria(
        str(root), str(file), file_extensions={".json", ".txt"}
    )
