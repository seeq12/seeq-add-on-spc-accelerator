...
def do_stuff(*args):
    ...
    units_lookup_dict = {'second(s)': 's', 'minute(s)': 'min', 'hour(s)': 'h', 'day(s)': 'd'}
...
def create_mean_formula_string(capsule_start, capsule_end, input_condition, capsule_property, conditions, start_select, end_select):
...
def push_signals(push_df, workbook_id, worksheet_name):
    push_results = spy.push(metadata=push_df, workbook=workbook_id, worksheet=worksheet_name, quiet=True)
    return push_results
...
def create_mean_and_stddev_signals(average_formula_string, stddev_formula_string, signal_name, input_condition, signals, conditions, apply_to_condition):
    if isinstance(input_condition.v_model, str):
        mean_stddev_signal_df = pd.DataFrame([{
            ...
            'Formula Parameters': {
                '$inputcondition': conditions[conditions['Name'] == input_condition.v_model],
...
def create_histogram(signal_df, start_time, end_time, input_condition, conditions, workbook_id, capsule_property):
    formulaAPI = sdk.FormulasApi(spy.client)
    itemsAPI = sdk.ItemsApi(spy.client)
