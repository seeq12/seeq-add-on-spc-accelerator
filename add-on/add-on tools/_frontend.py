...
def frontend(signal_list, condition_list, start_time, end_time):
    ...
    signal_interpolation = v.TextField(v_model=40, type='number', label='Maximum Signal Interpolation', style_="width: 200px")
    interpolation_units = v.Select(items=['second(s)', 'minute(s)', 'hour(s)', 'day(s)'], v_model='hour(s)', style_="width: 200px")

    input_condition = v.Select(
        v_model=condition_list, 
        items=condition_list, 
        label='(Optional) Condition Filter', 
        clearable=True, style_="width: 400px", 
    ...
    start_select = ipydatetime.DatetimePicker(description='Start Time: ', value=start_time)
    ...
    workbook_button = v.Btn(children=[v.Icon(children=['mdi-open-in-new'])], color='primary', target='_blank', disabled=True)
    error = v.Alert(type='error', dense=True, value=False, children=['Training Window: End time must be after start time.'])
    success = v.Alert(type='success', dense=True, value=False, children=[])
    
    return (
        input_signal, signal_interpolation, interpolation_units, input_condition, 
        capsule_property, start_select, end_select, apply_to_condition, 
        control_chart, we_runrules, nelson_runrules, histogram, 
        button, workbook_button, error, success
    )
